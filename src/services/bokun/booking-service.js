...
export class BokunBookingService {
    ...
    async getAllBookings(tourType) {
        try {
            console.log(`🔍 Fetching all bookings for ${tourType}`);
            ...
            console.log(`📋 Found ${bokunBookings.length} external Bokun bookings`);
            ...
            console.log(`✅ Total unique bookings: ${result.length}`);
    ...
    async getBokunBookings(tourType) {
        try {
            ...
            return bookings.map(booking => this.transformBokunBooking(booking, productMapping));
        } catch (error) {
