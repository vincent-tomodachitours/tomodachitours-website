...
interface ConversionTestResults {
    [key: string]: {
        testId: string | null;
        success: boolean;
    };
...
interface DiagnosticReport {
    ...
    gtmConfiguration: {
        ...
        containerValidation: ValidationResult;
    };
...
class GTMTestingUtils {
     ...
     */
    enablePreviewMode(containerId: string, previewToken: string): boolean {
        ...
        try {
            ...
            const gtmScript = document.querySelector(`script[src*="${containerId}"]`) as HTMLScriptElement;
            if (gtmScript) {
     ...
     */
    testConversionTagFiring(conversionType: string, testData: any = {}): string | null {
     ...
     */
    testAllConversions(): ConversionTestResults {
        const results: ConversionTestResults = {};
     ...
     */
    validateGTMLoading(containerId?: string): ValidationResult {
     ...
     */
    monitorDataLayerEvents(duration: number = 30000): MonitoredEvent[] {
        if (!window.dataLayer) {
            console.warn('GTM Testing: dataLayer not found');
            return [];
        }

        const originalPush = window.dataLayer.push;
        const monitoredEvents: MonitoredEvent[] = [];
