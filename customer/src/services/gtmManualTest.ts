...
declare global {
    interface Window {
        testGTMConversions: () => Promise<TestResults>;
        testAddPaymentInfoConversion: () => void;
        testPurchaseConversion: () => void;
        validateGTMSetup: () => any;
        monitorDataLayerEvents: (duration?: number) => any;
        testAllConversions: () => any;
    }
}

// Test GTM conversion tracking manually
window.testGTMConversions = async (): Promise<TestResults> => {
...
window.testAddPaymentInfoConversion = (): void => {
...
window.testPurchaseConversion = (): void => {
...
window.monitorDataLayerEvents = (duration: number = 30000): any => {
