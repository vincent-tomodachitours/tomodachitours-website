...
class DynamicRemarketingService {
    ...
    getSessionData() {
        try {
            ...
            return {
                sessionId,
                userId: localStorage.getItem('user_id') || sessionId,
                startTime: Date.now()
            };
        } catch (error) {
    ...
    fireDynamicRemarketingEvent(eventType, parameters) {
        try {
            // Use global gtag function (available in test environment)
            if (typeof gtag !== 'undefined') {
                // Fire Google Ads dynamic remarketing event
                ...
                // Also fire to Google Analytics for cross-platform tracking
    ...
    addUserToDynamicAudiences(userId, tourId, parameters, customAudienceId = null) {
        if (customAudienceId) {
            ...
            if (audience) {
                ...
                this.fireDynamicRemarketingEvent('audience_add', {
                    ...parameters,
