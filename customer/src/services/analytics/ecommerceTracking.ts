...
import { getShouldTrack, getShouldTrackMarketing, isTestEnvironment } from './config';
import { getTourCategory, getTourDuration, getTourLocation, getPriceRange, getUserEngagementLevel, storeUserInteraction } from './helpers';
import { storeCartData, clearCartData } from './cartTracking';
import gtmService from '../gtmService';
import bookingFlowManager from '../bookingFlowManager';
import tourSpecificTracking from '../tourSpecificTracking/index';
import attributionService from '../attributionService';
import remarketingManager from '../remarketingManager';
import dynamicRemarketingService from '../dynamicRemarketingService';
import { TransactionData, TrackingData } from '../../types';

// Enhanced ecommerce events for tour bookings
export const trackPurchase = async (transactionData: TransactionData): Promise<boolean> => {
    if (!getShouldTrack() || !transactionData) return false;

    try {
        // Get attribution data (skip in test environment)
        const attributionData: any = isTestEnvironment ? {} : attributionService.getAttributionForAnalytics();
        ...
        const conversionSuccess = gtmService.trackPurchaseConversion(gtmEventData, customerData || undefined);
...
export const trackBeginCheckout = async (tourData: TrackingData): Promise<boolean> => {
    if (!getShouldTrack() || !tourData) return false;
    ...
    try {
        ...
        const conversionSuccess = gtmService.trackBeginCheckoutConversion(gtmEventData, customerData || undefined);
...
export const trackTourView = async (tourData: TrackingData): Promise<boolean> => {
    if (!getShouldTrack() || !tourData) return false;
...
export const trackAddToCart = async (tourData: TrackingData): Promise<boolean> => {
    if (!getShouldTrack() || !tourData) return false;
...
// Track view item events (alias for trackTourView to match interface)
export const trackViewItem = async (tourData: TrackingData): Promise<boolean> => {
    return trackTourView(tourData);
};