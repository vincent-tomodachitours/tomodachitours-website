...
import { getShouldTrack } from './analytics/config.js';
import { getTourCategory, getTourLocation } from './analytics/helpers.js';
...
const AUDIENCE_CONFIGS = {
    ...
    HIGH_ENGAGEMENT: {
        ...
        criteria: {
            events: ['view_item', 'view_pricing', 'contact_whatsapp', 'contact_phone'],
            minEvents: 3,
            excludeEvents: ['purchase'],
            timeWindow: 72
        }
...
class RemarketingManager {
     ...
     * @param {string} audienceId - Audience identifier
     * @param {Object} eventData - Event data that triggered the audience addition
     */
    addUserToAudience(userId, audienceId, eventData = {}) {
     ...
     * @param {string} userId - User identifier
     * @returns {Array} Array of audience segments
     */
    getAudienceSegments(userId = null) {
        if (!userId) {
     ...
     * @param {Object} cartData - Cart data from abandonment event
     */
    processCartAbandonment(cartData) {
     ...
     * @param {Object} checkoutData - Checkout data from abandonment event
     */
    processCheckoutAbandonment(checkoutData) {
        if (!getShouldTrack()) return;
     ...
     */
    processPurchaseCompletion(purchaseData) {
        if (!getShouldTrack()) return;
    ...
    trackEngagementEvent(eventType, eventData) {
        ...
        let engagementData = JSON.parse(localStorage.getItem(engagementKey) || '{}');
        ...
        if (recentEvents.length >= 3) {
            this.addUserToAudience(userId, AUDIENCE_CONFIGS.HIGH_ENGAGEMENT.id, {
                ...
                engagementTypes: [...new Set(recentEvents.map(e => e.type))]
            });
        }
     ...
     * @param {Object} eventData - Event data
     */
    fireRemarketingEvent(audienceId, eventData) {
        try {
            // Fire Google Ads remarketing pixel
            if (window.gtag) {
                window.gtag('event', 'remarketing_audience_add', {
     ...
     */
    persistAudienceConfig(audience) {
        try {
     ...
     * @param {Object} eventData - Event data
     */
    persistUserAudience(userId, audienceId, eventData) {
        try {
     ...
     * @param {string} audienceId - Audience identifier
     */
    removePersistedUserAudience(userId, audienceId) {
        try {
            const userAudienceKey = `user_audiences_${userId}`;
            const userAudiences = JSON.parse(localStorage.getItem(userAudienceKey) || '{}');
     ...
     * @returns {Object} Audience statistics
     */
    getAudienceStats() {
        const stats = {};
