...
class ShoppingConversionService {
    ...
    /**
     * Track Shopping campaign click (from Shopping ad to product page)
     * @param {string} tourKey - Tour identifier
     * @param {Object} clickData - Click data from Shopping campaign
     */
    async trackShoppingClick(tourKey, clickData = {}) {
        try {
            // Get tour data
            const tourData = await getTour(tourKey);
            if (!tourData) {
            ...
            // Prepare Shopping click event data
            const clickEventData = {
                event_category: 'shopping_campaign',
                event_label: `shopping_click_${tourKey}`,

                // Shopping campaign data
                campaign_id: clickData.campaign_id,
                ad_group_id: clickData.ad_group_id,
                product_id: `tour_${tourKey}`,
                merchant_id: this.merchantId,

                // Product data
                product_title: tourData['tour-title'],
                product_price: tourData['tour-price'],
                product_category: this.productCategories[tourKey]?.category,

                // Attribution data
                gclid: clickData.gclid,
                gbraid: clickData.gbraid,
                wbraid: clickData.wbraid,
            ...
            // Push Shopping click event to GTM
            gtmService.pushEvent('shopping_click', clickEventData);

            console.log(`Shopping Conversion: Tracked shopping click for ${tourKey}`);
            return true;

        } catch (error) {
            console.error('Shopping Conversion: Error tracking shopping click:', error);
            return false;
        }
    }
    ...
    async _trackShoppingSpecificConversion(conversionType, tourKey, productData, shoppingData) {
        try {
            // Prepare Shopping-specific event data
            const shoppingEventData = {
                event: 'shopping_conversion',
                conversion_type: conversionType,
                conversion_label: this.conversionLabels[conversionType],
                ...
                // Conversion value
                conversion_value: productData.value,
                currency: productData.currency,
                ...
                // Tour specific
                tour_type: tourKey,
                tour_location: productData.location,
                booking_date: productData.booking_date,
                booking_time: productData.booking_time,

                // Additional Shopping data
                feed_id: shoppingData.feed_id,
                feed_item_id: shoppingData.feed_item_id,
                feed_country: 'JP',
                feed_language: 'en'
            };

            // Push to GTM dataLayer
            gtmService.pushEvent('shopping_conversion', shoppingEventData);
