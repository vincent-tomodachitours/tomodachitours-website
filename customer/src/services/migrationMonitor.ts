...
class MigrationMonitor {
    ...
    constructor() {
        this.monitoringEnabled = migrationFeatureFlags.getMigrationStatus().flags.migrationMonitoringEnabled;
     ...
     */
    async checkGTMContainer(): Promise<CheckResult> {
    ...
    checkDataLayer(): CheckResult {
        ...
        try {
            ...
            check.details.dataLayerLength = window.dataLayer ? window.dataLayer.length : 0;
            check.details.recentEvents = window.dataLayer ? window.dataLayer
                .slice(-10)
                .filter(item => item.event)
                .map(item => item.event) : [];

            // Check for recent conversion events
            const recentConversions = window.dataLayer ? window.dataLayer
            ...
            const testEventFound = window.dataLayer ? window.dataLayer.some(item =>
     ...
     * Check conversion tracking accuracy
     */
    checkConversionAccuracy(): CheckResult {
        const check: CheckResult = {
            status: 'healthy',
            details: {},
            issues: []
        ...
        try {
            ...
            check.details.discrepancyRate = validationSummary.discrepancyRate;
     ...
     */
    checkParallelTracking(): CheckResult {
        const check: CheckResult = {
            status: 'healthy',
            details: {},
            issues: []
    ...
    checkErrorRates(): CheckResult {
        ...
        try {
            ...
            if (parseFloat(check.details.errorRate as string) > 10) {
                check.status = 'critical';
                check.issues.push(`High error rate: ${check.details.errorRate}%`);
            } else if (parseFloat(check.details.errorRate as string) > 5) {
                check.status = 'warning';
     ...
     */
    calculateOverallHealth(checks: Record<string, CheckResult>): 'healthy' | 'warning' | 'critical' {
     ...
     */
    generateAlerts(checks: Record<string, CheckResult>): Alert[] {
        const alerts: Alert[] = [];
     ...
     */
    checkEmergencyRollbackConditions(alerts: Alert[]): void {
