...
export const useAvailability = (sheetId, availableTimes, maxSlots, participantsByDate, cancellationCutoffHours, cancellationCutoffHoursWithParticipant, nextDayCutoffTime) => {
     ...
     */
    const getDatabaseAvailabilityForMusicTour = useCallback(async (dateStr) => {
        try {
            ...
            // Parse time slots from database
     ...
     */
    const isDateFull = useCallback((date, participants) => {
        const formattedDate = date.toLocaleDateString("en-CA");
        const dayData = participantsByDate[formattedDate];
        const preloadedData = preloadedAvailability[formattedDate];
