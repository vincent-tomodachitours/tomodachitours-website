...
const BookingCancellation: React.FC = () => {
    ...
    const getTourName = (tourType: string): string => {
        ...
        const tourKey = tourKeyMap[tourType];

        return tourKey ? tours[tourKey]?.['tour-title'] || tourType : tourType;
    };
    ...
    const getTourUrl = (tourType: string): string => {
    ...
    const handleCancelBooking = async (booking: Booking) => {
        ...
        try {
            ...
            if (result.success) {
                ...
                try {
                    ...
                    trackCustomGoogleAdsConversion('booking_cancellation', {
                        ...
                        days_before_tour: Math.ceil((getBookingDateTime(booking).getTime() - Date.now()) / (24 * 60 * 60 * 1000)),
            ...
            setMessage('Failed to cancel booking: ' + errorMessage);
        } finally {
    ...
    const futureBookings = bookings
        .filter(booking => getBookingDateTime(booking).getTime() > now.getTime() && booking.status !== 'CANCELLED' && booking.status !== 'REFUNDED')
        .sort((a, b) => {
                            ...
                            {!tours ? (
                                ...
                                <div className="space-y-6">
                                    {futureBookings.map((booking) => {
                                        ...
                                        const canCancel = (bookingDateTime.getTime() - now.getTime()) > (24 * 60 * 60 * 1000);
                                        const tourName = getTourName(booking.tour_type);
                                        const refundAmount = calculateRefundAmount(booking);
