...
import { ImageShowcaseProps } from '../../types';

const ImageShowcase = ({ isMobile, images, tourId, tourName }: ImageShowcaseProps) => {
    const [imageOpen, setImageOpen] = useState(false);

    // Handle image click tracking
    const handleImageClick = (imageIndex: number = 0, _clickType: string = 'main_image') => {
        if (tourId && tourName) {
            trackTourImageClick(tourId, imageIndex);
        }
        setImageOpen(true);
    };
