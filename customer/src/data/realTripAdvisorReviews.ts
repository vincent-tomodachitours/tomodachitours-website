import type { RealBusinessInfo, Review } from '../types/data';
...
export const REAL_BUSINESS_INFO: RealBusinessInfo = {
    locationId: '27931661',
...
export async function getRealBusinessInfoWithAPI() {
    try {
        ...
        const proxyUrl = `${supabaseUrl}/functions/v1/tripadvisor-proxy`;
        console.log('🌐 Using TripAdvisor proxy:', proxyUrl);
        ...
        try {
            ...
            console.log('✅ TripAdvisor data fetched via proxy');
            return data;

        } catch (error) {
            console.warn('TripAdvisor proxy failed, using fallback:', error);
            return getRealBusinessInfo();
        }
    } catch (error) {
        console.warn('Failed to fetch TripAdvisor data, using fallback:', error);
        return getRealBusinessInfo();
    }
}
 ...
 */
export function getTourSpecificReviews(tourId: string, maxReviews: number = 6): Review[] {
